        ...
        if self.offer_type == "role":
            ...
            if role:
                ...
                await interaction.response.send_message(
                    f"✅ Tu as reçu le rôle {role.mention} !", ephemeral=True)
            ...
            if bot.shop_channel_id:
                ...
                await salon.send(
                    f"📦 {interaction.user.mention} a acheté `{self.label}`.")
            await interaction.response.send_message("✅ Achat effectué !",
                                                    ephemeral=True)
...
class ShopView(discord.ui.View):
    ...
    async def shorts(self, interaction, button):
        offers = [
            ...
            {
                "label": "1 clip + musique",
                "price": 90,
                "offer_type": "notif"
            },
            {
                "label": "2 clips + musique",
                "price": 120,
                "offer_type": "notif"
            },
        ]
        await interaction.response.send_message("Choisis ton offre :",
                                                view=OfferView(offers),
                                                ephemeral=True)
    ...
    async def role(self, interaction, button):
        offers = [
            {
                "label": "Rôle 1 semaine",
                "price": 20,
                "offer_type": "role",
                "duration": 7 * 24 * 3600,
                "role_id": 1279004230337822773
            },
            {
                "label": "Rôle 1 mois",
                "price": 50,
        ...
        await interaction.response.send_message("Choisis ton rôle :",
                                                view=OfferView(offers),
                                                ephemeral=True)
